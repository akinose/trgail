...
class ThrowerGoalEnv(mujoco_env.MujocoEnv, utils.EzPickle):
    def __init__(self):
        utils.EzPickle.__init__(self)
        self.goal = False
        self._ball_hit_ground = False
        self._ball_hit_location = None
        mujoco_env.MujocoEnv.__init__(self, 'throwergoal.xml', 5)
    ...
    def step(self, a):
        ...
        if not self._ball_hit_ground and self.get_body_com("ball")[2] < -0.25:
            ...
            self._ball_hit_location = self.get_body_com("ball").copy()
        ...
        #if self._ball_hit_ground and -np.linalg.norm(ball_hit_xy - goal_xy)
        if self._ball_hit_ground and -np.linalg.norm(ball_hit_xy - goal_xy) > -0.07:
            self.goal = True
#            achieve_reward = 1
#       else:
#            achieve_reward = 0
        ...
        print(self._ball_hit_ground,self._ball_hit_location)
        return ob, achieve_reward, done, dict(reward_dist=reward_dist,
                reward_ctrl=reward_ctrl, shaped_reward=reward)
