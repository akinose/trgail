...
class StrikerGoalEnv(mujoco_env.MujocoEnv, utils.EzPickle):
    def __init__(self):
        self.goal = False
        ...
        mujoco_env.MujocoEnv.__init__(self, 'strikergoal.xml', 5)
    ...
    def step(self, a):
        ...
        if self._striked:
            ...
            reward_near = - np.linalg.norm(vec_3) # 打った後: オブジェクトと打った場所の距離
        else:
            reward_near = - np.linalg.norm(vec_1) # 打つ前: オブジェクトとアームの距離

        reward_dist = - np.linalg.norm(self._min_strike_dist) #オブジェクトとゴールの最小距離
        reward_ctrl = - np.square(a).sum()
        reward = 3 * reward_dist + 0.1 * reward_ctrl + 0.5 * reward_near
 
        if np.linalg.norm(vec_2) < 0.08: # オブジェクトとゴールの現在距離が一定距離以内なら報酬
            self.goal = True
            achieve_reward = 1
        else:
            achieve_reward = 0
